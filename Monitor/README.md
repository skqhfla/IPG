...
`run_meta.json`이 있는 **런 출력 디렉토리**(예: `outputs_APK/<App>/<timestamp>/`)를 폴더로 선택하면 사이드바에 **🚀 이 런 열기** 버튼이 활성화됩니다. 클릭하면 4개 탭이 있는 Run Dashboard가 열립니다.
...
`/api/run/detect`, `/api/run/meta`, `/api/run/app-memory`, `/api/run/screen-memory`, `/api/run/log?filter=<scroll|executor|a11y|warning|all>`, `/api/run/snapshot?id=<snapshotId>`.
